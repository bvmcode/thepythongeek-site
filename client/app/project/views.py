...
class Database:
  ...
  def get_current_max_temp(self, date):
    ...
    query = f"""
            ...
            """
    df = pd.read_sql(query, con=engine)
    df["date"] = df["api_datetime"].dt.tz_localize('UTC').dt.tz_convert('America/New_York')
    engine.close()
...
def date_range():
    ...
    dt_now = utc_date.astimezone(pytz.timezone('America/New_York')).date()
...
def project4():
    utc_date = datetime.utcnow().replace(tzinfo=pytz.utc)
    dt_time = utc_date.astimezone(pytz.timezone('America/New_York'))
    dt = dt_time.date()
